function mainEvalBar() {
  ...
  function calcAccuracy(classificationsArr) {
    ...
    for (let i = 0; i < classificationsArr.length; i++) {
      ...
      if (classif.includes("best") || classif.includes("great") || classif.includes("mate") || classif.includes("draw")) {
        // no need to subtract
        foundClassif = true;
  ...
  function updateEvalBarSize() {
    ...
    evalCanvas.width = evalCanvasWidth;
    /* board text offset */
    let textOffset = 6;
    ...
    if (currHeight < defaultHeight) {
      let differenceMultiplier = currHeight / defaultHeight;
      textOffset = textOffset * differenceMultiplier;
    }
    /* boardGUI dimensions */
...
function checkDeviceScreen() {
  if (window.innerHeight > window.innerWidth && !alreadyAlerted) {
    alert("Unsupported screen resolution detected\nRotate your screen and refresh the page");
    alreadyAlerted = true;
  }
}
document.addEventListener("DOMContentLoaded", () => {
  mainEvalBar();
  setInterval(checkDeviceScreen, 100);
});
